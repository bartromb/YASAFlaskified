...
|---|---|---|
| YASAFlaskified | **v0.18.1** | `version.py` + `APP_VERSION` in `.env`; Docker image `yasaflaskified:0.18.1` (deployed 2026-08-03, psgscoring 0.14.1). v0.18.1 = **twee rapportgetallen die het verkeerde lazen, plus een derde v3-profiel**. (a) De rij "Rule 1B / CMS — ≥30% flow + ≥4% desat" vulde zich met de AHI van de `strict`-arm van de robuustheidssweep, maar `aasm_v3_strict` is een conservatieve variant van Rule *1A* (3%-of-arousal blijft): er is nooit een 4%-criterium toegepast op dat getal, in geen enkel rapport sinds v0.15.0. Rule 1B komt nu uit de eventlijst (alle apneus + hypopnees met ≥4% desat) en kan noch boven Rule 1A, noch onder de apneu-index uitkomen; zonder SpO₂ verdwijnt de rij. **Geen scoringswijziging** — kop-AHI, OAHI, ernstgradatie, besluit en eventtellingen lezen dit veld niet. (b) De AHI-sweep, anker-basislijn, arousal/RERA-koppeling, CSR-detectie en ventilatory burden lazen alle vijf het *apneukanaal*; onder `aasm_v3_dual` is dat de thermistor, ook wanneer die de kwaliteitstoets niet haalt. Nieuw profielveld `flow_reference` (default `"apnea"` = ongewijzigd). Op de aanleidende opname las de VB 20,4% i.p.v. 42,6%. (c) Nieuw in de dropdown: **`AASM v3 — Rule 1A, nasal-pressure reference`** (`aasm_v3_pressure`) — apneus zoals `Recommended`, afgeleide analyses op neusdruk; identiek aan `Recommended` op elke montage zonder bruikbare thermistor. Databehoud geverifieerd: 15 jobs / 6 users / 390 uploads (27G) voor en na; live op https://slaapkliniek.be/ (v0.18.1). Earlier: v0.18.0 (deployed 2026-08-03, psgscoring 0.14.0) = **het dual-sensorprofiel, kiesbaar in de UI**. `AASM v3 — Rule 1A, dual-sensor apneas` staat in de profielkeuze en scoort apneus op *beide* flowsensoren; geen van beide kanalen kan de andere overrulen. Elke apneu draagt `both` / `thermistor_only` / `pressure_only` in het rapport, met de noot dat enkelsensor-events **niet worden afgewezen** — de kolom is informatie, geen filter (op MESA ziet slechts 19% van 1785 apneudetecties beide sensoren). "Dual-sensor gevraagd maar niet uitgevoerd" komt in het Aandachtspunten-kader. **De standaardscoring verandert NIET**: `aasm_v3_rec` blijft default en is byte-identiek. Databehoud geverifieerd: 13 jobs / 6 users / 378 uploads (26G) voor en na; live op https://slaapkliniek.be/ (v0.18.0). ⚠️ **v0.17.5 is op 2026-08-02 teruggerold naar v0.17.4** en heeft nooit langdurig gedraaid: psgscoring 0.13.2 maakte de thermistor onvoorwaardelijk de apneusensor, en op een echte opname met menselijke scoring zakten de apneus van 93 naar 0, de AHI van 26,2 naar 8,6 en de conclusie van *matig CSAS* — door de scorer bevestigd — naar *mild SAS*. Onderstaande v0.17.5-beschrijving is dus historie, niet de toestand. Earlier: v0.17.5 = **beide AASM-flowsensoren, selecteerbaar en zichtbaar**. psgscoring 0.13.2 laat de tweesensorregel daadwerkelijk vuren: apneu op de oronasale thermistor, hypopneu op de nasale druk. ⚠️ **VERANDERT DE SCORING** op elke opname met een thermistor die 0.13.1 niet herkende (Somnomedics `Flow Th.`) — apneus werden op neusdruk gescoord, die overdetecteert. Bestaande rapporten blijven, een heranalyse van dezelfde EDF geeft andere apneu-aantallen. Het kanaalformulier heeft nu eigen velden voor beide sensoren (generieke `flow` = terugval) en toont ze in het auto-detectieoverzicht; bij één sensor meldt het rapport welk kanaal gebruikt is en dat apneus op neusdruk kunnen overdetecteren. Op prod geverifieerd: apneu->Flow Th., hypopneu->Pressure Flow. Databehoud geverifieerd: 13 jobs / 6 users / 378 uploads (26G) voor en na. Earlier: v0.17.4 (deployed 2026-08-02). v0.17.4 = **fix: de subtyperij drukte een zwart blokje af**. De `↳` uit 0.17.3 zit niet in het lettertype, dus ReportLab viel terug op ■ — en ■ is in ditzelfde rapport de legenda-kleurmarkering (hypnogram-stadia, eventtypes), dus de rij las als een derde betekenis van hetzelfde symbool. Nu `·`. Geverifieerd door in de PRODUCTIECONTAINER een rapport te genereren en de tekstlaag terug te lezen: `· waarvan centraal` / `· waarvan gemengd`, geen blokje. Databehoud geverifieerd: 13 jobs / 6 users / 378 uploads (26G) voor en na. Earlier: v0.17.3 (deployed 2026-08-02). v0.17.3 = **de herstelde event-velden bereiken nu het rapport**. Geen scoringswijziging. "Laagste SpO2 tijdens een event" komt in de saturatietabel als aparte regel naast het nachtminimum (dat kan van een artefact komen; deze waarde hoort bij een gescoord event), en de hypopnee-subtypering verschijnt als ingesprongen rijen `↳ waarvan centraal` / `↳ waarvan gemengd` onder de hypopnee-rij met dezelfde sterrenkolommen. Beide alleen getoond wanneer er iets te tonen valt; in nl/fr/en/de. Geverifieerd door `test_pdf_event_fields.py`, dat de PDF echt genereert en de tekstlaag terugleest - dezelfde test zou de foutieve renderingclaim in v0.17.2 gevangen hebben. Databehoud geverifieerd: 13 jobs / 6 users / 378 uploads (26G) voor en na. Earlier: v0.17.2 (deployed 2026-08-02). v0.17.2 = **psgscoring 0.13.1 + voetnoot bij de sterrenkolommen**. Geen scoringswijziging; elk profiel byte-identiek. De kop "★★★ ≥0.85" is een kaal getal dat als percentage gelezen wordt - gemeten tegen de twaalf scorers van PSG-IPA is de correlatie r = 0,19 en ligt het niveau ruim 30 procentpunt te hoog (een event met ★★★ wordt door ~44% gemarkeerd). De voetnoot zegt nu dat het een rangschikking is, geen kans; in nl/fr/en/de. Verder komen `min_spo2` en de subtypering `hypopnea_central`/`_mixed` terug in de EVENT-RECORDS (EDF-viewer + `/api/edf/<job_id>/events/...`), die het breath-profiel stil liet vallen — NIET in de PDF: die toont bij "min SpO2" het nachtminimum uit de saturatiesamenvatting, en de respiratoire tabel heeft geen subtyperijen voor hypopnees. Op een echte opname geverifieerd: de enige tekstwijziging in het hele rapport is de voetnoot; alle klinische waarden (AHI 32,5, Regel 1B 13,0, RERA 75, RDI 52,0, REM/NREM, artefacten) zijn identiek aan 0.17.1. Databehoud geverifieerd: 13 jobs / 6 users / 378 uploads (26G) voor en na. Earlier: v0.17.1 (deployed 2026-08-01). v0.17.1 = **psgscoring pin 0.12.1 -> 0.13.0**, waarmee het profiel **"AASM v3 — Rule 1A, breath-graded"** in de scoringprofiel-keuze verschijnt. Geen template- of routewijziging: `channel_select.html` bouwt het menu uit de psgscoring-registry. **Niet voorgeselecteerd** — de app geeft `scoring_profile="standard"` expliciet mee, dus de analyse verandert niet tenzij een gebruiker het profiel kiest. Wel te weten: een kanaal genaamd `Pres` wordt nu als neusdruk herkend in plaats van als pulse (hypopnees werden daarvoor van de thermistor gescoord) — gewone klinische PSG's zonder zo'n kanaal veranderen niet, NSRR/MESA-opnames wel; en arousals bereiken de EDF+-export en de event-API weer (psgscoring issue #16), additief want *reageren* op die lijst is een profielkeuze die alleen het nieuwe profiel aanzet. Databehoud geverifieerd: 13 jobs / 6 users / 378 uploads (26G) voor en na. Earlier: v0.17.0 (deployed 2026-07-31) = **job registry + upload/access hardening**: path traversal in the chunked upload closed (`file_id` allowlist + upload-dir containment); job authorisation moved from JSON files on disk into a `job` table, enforced by `@job_access_required` on all 30 `<job_id>` routes (8 previously had none); idempotent `backfill_jobs`. Backfill on deploy: **13 studies found, 13 rows created, 0 without owner**. `JOB_ACCESS_STRICT` is **not set -> default "0" (permissive JSON fallback)**; raise to `"1"` once `job access fallback` stays absent from the logs. **No scoring change** — psgscoring pin unchanged at 0.12.1, no scoring/report/i18n file touched. Earlier: v0.16.5 (deployed 2026-07-27). v0.16.5 = **ventilatory-burden '≤25%' reference hidden for central-dominant studies (>50% central events / CSAS)** — the VB norm (AJRCCM 2023) is obstructive-OSA-derived and inherently high in central apnea/Cheyne-Stokes; the VB value stays shown (`_is_central_dominant` in generate_pdf_report + Jinja in results_extended). Also: absolute DISCLAIMER link in README (fix dead PyPI link). v0.16.4 = **ML `ml_help` tooltip de-detailed** (dropped 'paper v35 §3.6.1' + model version tag). v0.16.3 = **disclaimer 'Vallat & Walker' backslash fix + landing-page publication de-detailed (no journal/version/authors) + 'AASM v3.0' roadmap card removed (done)**. v0.16.2 = **landing page refreshed** (What's New → AASM v3, dual AHI, burdens, phenotypes, clinician report, arousal aetiology; badge/stack updated). v0.16.1 = **ventilatory burden made breath-based** (psgscoring 0.12.1; pin → 0.12.1): VB is now the proportion of breaths whose peak amplitude is <50% of the eupneic baseline (v0.16.0 used the envelope time-fraction → over-counted, e.g. 82.9%). Report display unchanged (bounded `%`, ref ≤25%). Earlier: v0.16.0 = **VB recalibration + saturation bands + arousal-aetiology fix** (psgscoring 0.12.0; pin → 0.12.0): ventilatory burden now the validated % of "small breaths" (airflow <50% eupneic, AJRCCM 2023) shown as bounded `%` with ref ≤25% (was implausible `%·min/h`); time-in-saturation-bands table now populated; respiratory+spontaneous arousal indices now sum to the arousal index. No AHI/OSAS-grade change. Earlier: v0.15.0 = **clinician-focused PDF report** (from psgscoring 0.11.0; pin → 0.11.0): auto-generated conclusion, dual AHI (Rule 1A vs 1B/CMS 4%), AASM AHI reference scale, page-1 phenotype line + descriptive "Aandachtspunten" box, arousal aetiology indices, ventilatory burden marked experimental. **Removed from the PDF:** signal-quality/confidence section + banners, and the "OSAS severity profile" (strict/std/sensitive comparison, AHI-robustness interval, OAHI 3-point sweep, O-S-A-S score table) — ESS kept. **Fix:** KPI syndrome/apnea-breakdown now read the canonical summary (were empty → generic "SAS"). No scoring-numerics change. Earlier: v0.14.0 = phenotypes + ventilatory burden in report; v0.13.0 = arousal & RERA moved to psgscoring + multi-derivation arousals by default |
